...
export function PerfilUsuario() {
  ...
  const guardarDatos = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Error al guardar datos', { id: toastId })
      }
    } catch (error) {
  ...
  const getEstiloEstado = (estado) => {
    switch (estado) {
      ...
      default:
        return { bg: 'rgba(197, 160, 89, 0.1)', color: '#c5a059', border: '#c5a059' }
    }
  }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'60px 20px', gap:'40px', background:'#121212', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:'700px', width:'100%', background:'#1e1e1e', padding:'40px', borderRadius:'20px', border:'1px solid #333', boxShadow:'0 10px 30px rgba(0,0,0,0.3)' }}>
        <h2 style={{ color:'#c5a059', margin:'0 0 5px 0', fontSize:'1.5rem' }}>Datos del Perfil</h2>
        <p style={{ color:'#a0a0a0', marginBottom:'30px', fontSize:'0.95rem' }}>
          Tus datos predeterminados para agilizar tus compras.
        </p>

        {usuario && (
          <p style={{ color:'#a0a0a0', marginBottom:'20px' }}>
            Usuario: <strong style={{ color:'#fff' }}>{usuario.username}</strong>
            {' · '}
            Rol: <strong style={{ color:'#c5a059' }}>{usuario.role}</strong>
          </p>
        )}
        ...
        <form onSubmit={guardarDatos}>
          ...
          <div style={{ marginBottom:'15px' }}>
            <label style={{ color:'#fff', fontSize:'0.9rem', marginBottom:'8px', display:'block' }}>Email</label>
            <input
              type="email"
              name="email"
          ...
          <div style={{ marginBottom:'30px' }}>
            <label style={{ color:'#fff', fontSize:'0.9rem', marginBottom:'8px', display:'block' }}>Dirección</label>
            <input
              type="text"
              name="direccion"
          ...
          <div style={{ display:'flex', gap:'15px' }}>
            <button
              type="submit"
              style={{ flex:1, padding:'15px', fontSize:'1rem', fontWeight:'bold', background:'#c5a059', color:'#000', border:'none', borderRadius:'8px', cursor:'pointer', transition:'all 0.3s' }}
              onMouseOver={(e) => (e.target.style.background = '#e0b86a')}
            ...
            <button
              ...
              style={{ flex:1, padding:'15px', fontSize:'1rem', fontWeight:'bold', background:'transparent', color:'#fff', border:'1px solid #444', borderRadius:'8px', cursor:'pointer', transition:'all 0.3s' }}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'15px' }}>
            {pedidos.map((pedido) => {
              ...
              return (
                <div
                  key={pedido.id}
                  style={{ border:'1px solid #333', borderRadius:'12px', padding:'20px', display:'flex', justifyContent:'space-between', alignItems:'center', backgroundColor:'#111', transition:'transform 0.2s' }}
                  onMouseOver={(e) => (e.currentTarget.style.transform = 'translateX(5px)')}
                  onMouseOut={(e) => (e.currentTarget.style.transform = 'none')}
                >
                  <div>
                    <p style={{ margin:'0 0 5px 0', color:'#fff', fontWeight:'bold', fontSize:'1.1rem' }}>
                      Pedido #{pedido.id}
                    </p>
                    <p style={{ margin:'0 0 10px 0', color:'#a0a0a0', fontSize:'0.9rem' }}>
        ...
        <div style={{ position:'fixed', top:0, left:0, width:'100vw', height:'100vh', backgroundColor:'rgba(0,0,0,0.85)', backdropFilter:'blur(5px)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:9999 }}>
          <div style={{ backgroundColor:'#1e1e1e', padding:'40px', borderRadius:'20px', maxWidth:'500px', width:'90%', border:'1px solid #c5a059', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 50px rgba(0,0,0,0.5)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid #333', paddingBottom:'15px', marginBottom:'25px' }}>
              <h3 style={{ margin:0, color:'#fff', fontSize:'1.5rem', fontFamily:'Playfair Display' }}>
                Pedido <span style={{ color:'#c5a059' }}>#{pedidoSeleccionado.id}</span>
              </h3>

              <button
                onClick={() => setModalAbierto(false)}
                style={{ background:'transparent', border:'none', color:'#a0a0a0', fontSize:'1.5rem', cursor:'pointer' }}
              >
                ✕
              </button>
            </div>
            ...
            <div style={{ marginBottom:'25px', backgroundColor:'#111', padding:'15px', borderRadius:'8px', border:'1px solid #333' }}>
              <p style={{ margin:'0 0 8px 0', color:'#a0a0a0', fontSize:'0.9rem' }}>
                <strong>Estado:</strong>{' '}
                <span style={{ color:getEstiloEstado(pedidoSeleccionado.estado).color, fontWeight:'bold' }}>
              ...
              <p style={{ margin:'0 0 8px 0', color:'#a0a0a0', fontSize:'0.9rem' }}>
                ...
                <span style={{ color:'#fff' }}>{pedidoSeleccionado.clienteDireccion || 'Retiro en Local'}</span>
              </p>

              <p style={{ margin:'0', color:'#a0a0a0', fontSize:'0.9rem' }}>
                <strong>Teléfono:</strong>{' '}
                <span style={{ color:'#fff' }}>{pedidoSeleccionado.clienteTelefono || '-'}</span>
              </p>
            </div>
            ...
            <ul style={{ listStyle:'none', padding:0, margin:'0 0 25px 0' }}>
              {pedidoSeleccionado.items?.map((item, idx) => (
                <li
                  key={idx}
                  style={{ display:'flex', justifyContent:'space-between', padding:'12px 0', borderBottom:'1px solid #333', color:'#fff' }}
                >
                  <div style={{ flex:1 }}>
                    <span style={{ color:'#c5a059', fontWeight:'bold', marginRight:'8px' }}>
                      {item.cantidad}x
                    </span>
                    {item.nombre}
                  </div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:'20px', borderTop:'1px dashed #555' }}>
              <span style={{ margin:0, color:'#a0a0a0', textTransform:'uppercase', letterSpacing:'1px' }}>
                Total Abonado
              </span>
              <span style={{ margin:0, color:'#fff', fontSize:'1.8rem', fontWeight:'900' }}>
            ...
            <button
              onClick={() => setModalAbierto(false)}
              style={{ width:'100%', marginTop:'30px', padding:'15px', background:'#333', color:'#fff', border:'none', borderRadius:'8px', fontWeight:'bold', cursor:'pointer', transition:'background 0.3s' }}
